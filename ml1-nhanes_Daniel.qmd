...
pairs(BMI ~ Age + Gender + Pulse + BPSysAve + PhysActive + Race1 ,
      data = nhanes_sub, pch = ".", upper.panel = panel.smooth)
...
lm_bmi <- lm(BMI ~ Age + Gender + Pulse + BPSysAve + PhysActive + Race1, data = nhanes_sub)
summary(lm_bmi)
...
summary(nhanes_sub$AlcoholDay)

nhanes_sub %>%
  ggplot(aes(x = Age, y = log(AlcoholDay), color = Gender)) + 
  geom_point(alpha = 0.5) +
  geom_smooth()

glm_phys <- glm(AlcoholDay ~ Age + Gender + PhysActive + SleepTrouble + Race1,
                data = nhanes_sub, family = poisson)
summary(glm_phys)
